...
function Home() {
    ...
    const { stocks, stockData,loadingStockData } = useSelector(
    ...
    return (
        <>
            <div className="flex space-x-4 px-10 my-4">
                    ...
                    <SelectContent>
                        <SelectGroup>
                            ...
                            {selectedStock &&
                                ...
                                Object.entries(stocks.find(stock => stock.id.trim() === selectedStock.trim()).available as Record<string, string>)
                                    .map(([_,key]) => (
                                        <SelectItem key={key} value={key}>{String(key)}</SelectItem>
                                    ))
                            }
                        </SelectGroup>
                        </SelectContent>
                ...
                <Button variant="destructive" disabled={!selectedStock || !selectedDuration} onClick={fetchStockDetails}>
                    Submit
                </Button>
